...
int RXBitBoard::count_flips_AVX2(const int pos, unsigned long long P)
{
	...
	n_flipped  = COUNT_FLIP[h][(P >> (pos & 0x38)) & 0xFF];
	n_flipped += COUNT_FLIP[v][_pext_u64(P, MASK_X[pos][0])];
	n_flipped += COUNT_FLIP[v][_pext_u64(P, MASK_X[pos][1])];
	n_flipped += COUNT_FLIP[v][_pext_u64(P, MASK_X[pos][2])];
